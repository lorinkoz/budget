...
apache_template = '''
Alias /presupuesto/robots.txt "%(robots_path)s"
Alias /presupuesto/favicon.ico "%(favicon_path)s"

Alias /presupuesto/static "%(static_path)s"
<Directory "%(static_path)s">
    Order Allow,Deny
    Allow from all
</Directory>

WSGIDaemonProcess presupuesto python-path="%(project_path)s"
WSGIScriptAlias /presupuesto "%(wsgi_path)s" process-group=presupuesto application-group=%%{GLOBAL}
