...
class PlanArea(StaffRequiredMixin, TemplateView):
    ...
    def dispatch(self, request, slug, year):
        ...
        years = {"anterior": this_year - 1, "actual": this_year, "proximo": this_year + 1}
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.year == timezone.now().year:
            deactivate = [
                x.pk
                for x in self.area.destinations.filter(plans__year=self.year)
                .distinct()
                .annotate(plan=Sum("plans__amount"))
                .filter(plan=0)
            ]
...
class RecordList(StaffRequiredMixin, SetHeadlineMixin, ListView):
    ...
    def get_template_names(self):
        template = (
            "control/record_list_print.html" if self.request.GET.get("print", False) else "control/record_list.html"
        )
        return [template]

    def get_queryset(self):
        queryset = (
            super(RecordList, self)
            .get_queryset()
            .select_related("destination", "destination__area", "destination__element", "concept")
        )
...
class RecordCreate(StaffRequiredMixin, SetHeadlineMixin, CreateView):
    ...
    def get_initial(self):
        return {"currency": self.request.user.default_currency}
